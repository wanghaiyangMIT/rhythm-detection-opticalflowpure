...
class Shift(nn.Module):
    def __init__(self, args):
        ...
        self.group = [0, 500]    # how features group up
        self.n_group = len(self.group) - 1

        if self.use_attention:
            self.dim = args.dim_feature
    ...
    def forward(self, x):
        ...
        if self.use_attention:
            ...
            x_padded[:,self.p:self.T+self.p,:] = x # (n, T+p*2, D)
            ...
            for i in range(self.n_group):
                ...
                #     res[:,t,d] = torch.matmul(weight[:,t:t+1,:], x_padded[:,t:t+self.p*2+1,d]).squeeze(dim=1)
                attention = self.attentions[i]
                for t in range(self.T):
                    res[:,t:t+1,d] = attention(
                        x_padded[:,self.p+t:t+self.p+1,:],  # (n, 1,     D)
                        x_padded[:,t:t+self.p*2+1,:],       # (n, p*2+1, D)    
                        x_padded[:,t:t+self.p*2+1,d]        # (n, p*2+1, d[i]-d[i-1])
                    )
            x = res
        else:
            for i in range(self.n_group):
